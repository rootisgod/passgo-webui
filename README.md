...
### Graphical Console
- **Browser VNC console** powered by the bundled noVNC client
- **No guest network port** — PassGo tunnels raw RFB traffic through `multipass exec` to TigerVNC on guest loopback
- **Per-VM random password** generated by the built-in `ubuntu-desktop-novnc.yml` cloud-init template

Existing VMs created with the earlier noVNC template must be reprovisioned with the current template before the VNC tab can connect.
...
The server starts on `http://127.0.0.1:8080`. On first run it prints a one-time random password for the `admin` account and creates `~/.passgo-web/config.json`.
...
-username name  Override the login username
-password value Set the initial or override login password
...
| Setting | Description |
|---------|-------------|
| `listen` | Address and port (default `127.0.0.1:8080`) |
...
| **Graphical Console** | | |
| `/vms/{name}/vnc` | GET | Check VNC availability and obtain connection details |
| `/vms/{name}/vnc/websocket` | WebSocket | Authenticated raw RFB tunnel |
| **Cloud-Init** | | |
...
The service binds to `127.0.0.1:8080` by default and prints the initial password to the journal once. Use a reverse proxy for remote access, or explicitly change `listen` in `/root/.passgo-web/config.json` to a trusted interface.
