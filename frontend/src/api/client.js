...
export const installSSHKey = (name, publicKey = '') => request('POST', `/vms/${name}/ssh-key`, publicKey ? { public_key: publicKey } : {})
export const listVMTemplates = () => request('GET', '/vm-templates')
export const setVMTemplate = (name, template) => request('PUT', `/vms/${name}/template`, { template })
export const listProxyRules = (vm) => request('GET', `/proxy-rules${vm ? `?vm=${encodeURIComponent(vm)}` : ''}`)
export const createProxyRule = (rule) => request('POST', '/proxy-rules', rule)
export const cleanupProxyRules = () => request('POST', '/proxy-rules/cleanup')
export const updateProxyRule = (id, rule) => request('PUT', `/proxy-rules/${encodeURIComponent(id)}`, rule)
export const deleteProxyRule = (id) => request('DELETE', `/proxy-rules/${encodeURIComponent(id)}`)
