...
- **Name validation is mandatory before subprocess calls.** `pkg/multipass/constants.go` exports `ValidateVMName`, `ValidateGroupName`, `ValidateProfileID`, `ValidatePlaybookFilename`. Every `Client` method that passes a name to `exec.Command` validates first — this is what prevents flag injection like `DELETE /api/v1/vms/--all` from becoming `multipass delete --all`. Handlers also call `validVMName(w, r, "name")` (in `middleware.go`) for cleaner 400 errors at the HTTP boundary. Any new `multipass` call must follow this pattern.
- **Config saves must use `s.configPath` / `srv.configPath`, never `config.DefaultConfigPath()`.** The path is stored on `Server` so `-config /other/path.json` is honored. Only `cmd/server/main.go` calls `DefaultConfigPath()` as the flag default.
- `config.Save` is atomic: writes to `.config-*.tmp` in the same dir, fsyncs, then renames. A crash mid-save leaves the old file intact.
...
- LLM tool executor in `llm_executor.go`: switch dispatch from tool name to client method. Group tools use `cfgMu` mutex + `config.Save()`. Tool errors returned as JSON for LLM to explain, not Go errors.
- LLM config in `config.go`: `LLMConfig` struct with `base_url`, `api_key`, `model`, `read_only` fields, nested under `Config.LLM`
- Launch profiles in `config.go`: `Profile` struct with id, name, release, cpus, memory_mb, disk_gb, cloud_init, network, playbook, group. Stored as `profiles` array in config.json. CRUD methods on Config, protected by `cfgMu` mutex.
...
- API calls centralized in `api/client.js` using a single `request()` helper. **401 handling is centralised**: on any 401, `client.js` dispatches a `passgo:unauthorized` window event; `App.vue` listens, clears `store.authenticated`, and shows a toast. Individual callers should NOT re-implement logout on 401.
...
- LLM chat panel (`ChatPanel.vue`): right-side blade, slides in/out, SSE streaming via fetch ReadableStream. Chat history persisted to localStorage. Markdown rendering via `useMarkdown.js` — uses `marked` for parsing + `DOMPurify` for sanitisation (LLM output is untrusted; don't roll your own escaping). Any new `v-html` site that renders untrusted content must go through `renderMarkdown()` or an equivalent DOMPurify pass.
